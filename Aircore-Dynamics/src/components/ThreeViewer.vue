...
function focusOnProcess(processName) {
  const view = processName ? processCameraPositions[processName] : defaultCameraPosition;
...
function resetView() {
  ...
  focusOnProcess(null); // 'null'을 전달하여 defaultCameraPosition을 사용하도록 함
...
function focusNextEquipment() {
  ...
  const relevantObjects = props.highlightedProcess
    ? factoryObjects.children.filter(
        (m) => m.userData.Process_Name === props.highlightedProcess
      )
...
function createMachines() {
  ...
  props.machineInfo.forEach((data) => {
    ...
    switch (data.Process_Name) {
      case "주조":
        machine = createCastingMachine();
        break;
      case "가공":
        machine = createProcessingMachine();
        break;
      case "검사":
        machine = createInspectionMachine();
        break;
      case "조립":
        machine = createAssemblyMachine();
        break;
      case "포장":
        machine = createPackagingMachine();
        break;
    }
    if (machine) {
      ...
      if (data.Process_Name === "주조" || data.Process_Name === "검사") {
        towerLamp.position.set(2.5, 6, 0);
      } else {
        towerLamp.position.set(2.5, 5, 0);
      }
...
function createInspectionMachine() {
  ...
  const m = new THREE.Mesh(new THREE.BoxGeometry(6, 0.5, 4), b);
  m.position.y = 2;
  g.add(m);
  const a = new THREE.Mesh(new THREE.BoxGeometry(0.5, 3, 0.5), f);
  a.position.set(-2.5, 3.5, 0);
  g.add(a);
  const c = new THREE.Mesh(new THREE.BoxGeometry(5, 0.5, 0.5), f);
...
function createAssemblyMachine() {
  ...
  a.position.y = 2.5;
  g.add(a);
  const c = new THREE.Mesh(new THREE.BoxGeometry(2, 0.5, 0.5), f);
...
function createPackagingMachine() {
  ...
  const m = new THREE.Mesh(new THREE.BoxGeometry(10, 0.5, 3), b);
  m.position.y = 1;
  g.add(m);
